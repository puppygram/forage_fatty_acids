---
title: "Fatty acid composition of forages throughout the grazing interval "
...
```{r}
#Make y coordinates with 0 being the bottom left of pasture
y.cor.list <- 
  as.data.frame(seq(from = 14.45238, to = 607 , by = 28.90476)) %>%
  rename("y.cor" = "seq(from = 14.45238, to = 607, by = 28.90476)") %>%
  mutate(Paddock = c(7,7,7,6,6,6,5,5,5,4,4,4,3,3,3,2,2,2,1,1,1)) %>%
  mutate(Paddock = as.factor(Paddock)) %>%
  mutate(pen.row = rep(c(3,2,1), 7))

#read in data
dat <- 
  read_excel("C:/Users/Hannah/Desktop/Forage paper/Fatty acids/R_project/Fatty_acid_data.xlsx") %>% 
  mutate_at(vars(Forage, Paddock, Pen), as.factor) %>%
  mutate(x.cor = ifelse(Forage == "Wheat", 33, 99)) %>% #add x coordinates
  mutate(pen.row = ifelse(Pen == "1" | Pen == "2", 1, ifelse(Pen == "3" | Pen == "5", 2, ifelse(Pen == "4" | Pen  == "6", 3, NA))))

#merge with y coordinates
dat <- 
  as.data.frame(left_join(dat, y.cor.list)) %>%
  mutate(x.y.cor = as.factor(paste(x.cor, y.cor, sep = ","))) %>%
  mutate(pen_paddock = as.factor(paste(Pen, Paddock, sep = "_"))) %>%
  mutate(paddock_forage = as.factor(paste(Paddock, Forage, sep = "_")))

#make outcome variables on % scale
dat[,6:16] <- dat[,6:16]*100 #
```
...
newDat <- 
  data.frame(
    Day = rep(0:46, 2), 
    Forage = c(rep("Rye", 47), rep("Wheat", 47)), 
    Pen = NA, 
    Paddock = NA
  )
...
```{r}
#model
C140 <- lmer(C140 ~ Day*Forage + (1|Pen/Paddock) + (1|Paddock/Forage), 
             data = within(dat, Forage <- relevel(Forage, ref = "Rye")))

#diagnostics
plot(C140, sqrt(abs(residuals(.))) ~ fitted(.), type = c("p","smooth"))
qqmath(C140)

# check spatial correlation between the 42 pens
simulationOutput <- simulateResiduals(C140)
simulationOutputRecalc <- recalculateResiduals(simulationOutput, group = dat$pen_paddock)
testSpatialAutocorrelation(simulationOutputRecalc, 
                           x =  aggregate(dat$x.cor, list(dat$pen_paddock), mean)$x, 
                           y = aggregate(dat$y.cor, list(dat$pen_paddock), mean)$x, 
                           plot = F)
...

{
  ggplot(data = dat, 
    ...
    labs(x = "Grazing interval, day", 
         y = "Myristic acid, g 100g\u207B\u00B9 of total fatty acids", 
         caption = "Scatter plot of data with regression (solid lines) and bootstrap 95% confidence intervals (transparent bands) based on \n1000 resamples for myristic acid (C14:0) concentrations of fatty acids in rye and wheat forages during the grazing interval \n(day0 = 25 Apr; day46 = 10 Jun 2016).") +  
...
```{r}
#model
C160 <- lmer(C160 ~ Day*Forage + (1|Pen/Paddock) + (1|Paddock/Forage), 
             data = within(dat, Forage <- relevel(Forage, ref = "Wheat")))

#diagnostics
plot(C160, sqrt(abs(residuals(.))) ~ fitted(.), type = c("p","smooth"))
qqmath(C160)

# check spatial correlation between the 42 pens
simulationOutput <- simulateResiduals(C160)
simulationOutputRecalc <- recalculateResiduals(simulationOutput, group = dat$pen_paddock)
testSpatialAutocorrelation(simulationOutputRecalc, 
                           x =  aggregate(dat$x.cor, list(dat$pen_paddock), mean)$x, 
                           y = aggregate(dat$y.cor, list(dat$pen_paddock), mean)$x,
...
```{r}
#model
C180 <- lmer(C180 ~ Day*Forage + (1|Pen/Paddock) + (1|Paddock/Forage), 
             data = within(dat, Forage <- relevel(Forage, ref = "Wheat")))

#diagnostics
plot(C180, sqrt(abs(residuals(.))) ~ fitted(.), type = c("p","smooth"))
qqmath(C180)

# check spatial correlation between the 42 pens
simulationOutput <- simulateResiduals(C180)
simulationOutputRecalc <- recalculateResiduals(simulationOutput, group = dat$pen_paddock)
testSpatialAutocorrelation(simulationOutputRecalc, 
                           x =  aggregate(dat$x.cor, list(dat$pen_paddock), mean)$x, 
                           y = aggregate(dat$y.cor, list(dat$pen_paddock), mean)$x,
...
```{r}
{
  ggplot(data = dat, 
    ...
    labs(x = "Grazing interval, day", 
         ...
         caption = "Scatter plot of data with regression (solid lines) and bootstrap 95% confidence intervals (transparent bands) based on \n1000 resamples for stearic acid (C18:0) concentrations of fatty acids in rye and wheat forages during the grazing interval \n(day0 = 25 Apr; day46 = 10 Jun 2016).") +
...
```{r}
#model
C181.2 <- lmer(C181 ~ poly(Day, 2)*Forage + (1|Pen/Paddock) + (1|Paddock/Forage), 
               data = within(dat, Forage <- relevel(Forage, ref = "Wheat")))

#diagnostics
plot(C181.2, sqrt(abs(residuals(.))) ~ fitted(.), type = c("p","smooth"))
qqmath(C181.2)

# check spatial correlation between the 42 pens
simulationOutput <- simulateResiduals(C181.2)
simulationOutputRecalc <- recalculateResiduals(simulationOutput, group = dat$pen_paddock)
testSpatialAutocorrelation(simulationOutputRecalc, 
                           x =  aggregate(dat$x.cor, list(dat$pen_paddock), mean)$x, 
                           y = aggregate(dat$y.cor, list(dat$pen_paddock), mean)$x)
...

{
  ggplot(data = dat, 
    ...
    labs(x = "Grazing interval, day", 
         ...
         caption = "Scatter plot of data with regression (solid lines) and bootstrap 95% confidence intervals (transparent bands) based on \n1000 resamples for oleic acid (C18:1) concentrations of fatty acids in rye and wheat forages during the grazing interval \n(day0 = 25 Apr; day46 = 10 Jun 2016).") +  
...
```{r}
#model
C182.2 <- lmer(C182 ~ poly(Day, 2)*Forage + (1|Pen/Paddock) + (1|Paddock/Forage), 
               data = within(dat, Forage <- relevel(Forage, ref = "Wheat")))

#diagnostics
plot(C182.2, sqrt(abs(residuals(.))) ~ fitted(.), type = c("p","smooth"))
qqmath(C182.2)

# check spatial correlation between the 42 pens
simulationOutput <- simulateResiduals(C182.2)
simulationOutputRecalc <- recalculateResiduals(simulationOutput, group = dat$pen_paddock)
testSpatialAutocorrelation(simulationOutputRecalc, 
                           x =  aggregate(dat$x.cor, list(dat$pen_paddock), mean)$x, 
                           y = aggregate(dat$y.cor, list(dat$pen_paddock), mean)$x)
...
```{r}
#model
C183 <- lmer(C183 ~ Day*Forage + (1|Pen/Paddock) + (1|Paddock/Forage), 
             data = within(dat, Forage <- relevel(Forage, ref = "Rye")))

#diagnostics
plot(C183, sqrt(abs(residuals(.))) ~ fitted(.), type = c("p","smooth"))
qqmath(C183)

# check spatial correlation between the 42 pens
simulationOutput <- simulateResiduals(C183)
simulationOutputRecalc <- recalculateResiduals(simulationOutput, group = dat$pen_paddock)
testSpatialAutocorrelation(simulationOutputRecalc, 
                           x =  aggregate(dat$x.cor, list(dat$pen_paddock), mean)$x, 
                           y = aggregate(dat$y.cor, list(dat$pen_paddock), mean)$x,
...
#Difference between wheat - rye on d26
mean(data.frame(data.frame(boot.ci.C183)$X74 - data.frame(boot.ci.C183)$X27)[,1])
quantile(data.frame(data.frame(boot.ci.C183)$X74 - data.frame(boot.ci.C183)$X27)[,1], 0.025)
quantile(data.frame(data.frame(boot.ci.C183)$X74 - data.frame(boot.ci.C183)$X27)[,1], 0.975)
...
```{r}
#model
C200.2 <- lmer(C200 ~ poly(Day, 2)*Forage + (1|Pen/Paddock) + (1|Paddock/Forage), 
               data = within(dat, Forage <- relevel(Forage, ref = "Wheat")))

#diagnostics
plot(C200.2, sqrt(abs(residuals(.))) ~ fitted(.), type = c("p","smooth"))
qqmath(C200.2)

# check spatial correlation between the 42 pens
simulationOutput <- simulateResiduals(C200.2)
simulationOutputRecalc <- recalculateResiduals(simulationOutput, group = dat$pen_paddock)
testSpatialAutocorrelation(simulationOutputRecalc, 
                           x =  aggregate(dat$x.cor, list(dat$pen_paddock), mean)$x, 
                           y = aggregate(dat$y.cor, list(dat$pen_paddock), mean)$x, 
...

{
  ggplot(data = dat, 
    ...
    labs(x = "Grazing interval, day", 
         ...
         caption = "Scatter plot of data with regression (solid lines) and bootstrap 95% confidence intervals (transparent bands) based on \n1000 resamples for arachidic acid (C20:0) concentrations of fatty acids in rye and wheat forages during the grazing interval \n(day0 = 25 Apr; day46 = 10 Jun 2016).") +
...
```{r}
#model
C220 <- lmer(C220 ~ Day*Forage + (1|Pen/Paddock) + (1|Paddock/Forage), 
             data = within(dat, Forage <- relevel(Forage, ref = "Wheat")))

#diagnostics
plot(C220, sqrt(abs(residuals(.))) ~ fitted(.), type = c("p","smooth"))
qqmath(C220)

# check spatial correlation between the 42 pens
simulationOutput <- simulateResiduals(C220)
simulationOutputRecalc <- recalculateResiduals(simulationOutput, group = dat$pen_paddock)
testSpatialAutocorrelation(simulationOutputRecalc, 
                           x =  aggregate(dat$x.cor, list(dat$pen_paddock), mean)$x, 
                           y = aggregate(dat$y.cor, list(dat$pen_paddock), mean)$x,
...
```{r}
{
  ggplot(data = dat, 
    ...
    labs(x = "Grazing interval, day", 
         ...
         caption = "Scatter plot of data with regression (solid lines) and bootstrap 95% confidence intervals (transparent bands) based on \n1000 resamples for behenic acid (C22:0) concentrations of fatty acids in rye and wheat forages during the grazing interval \n(day0 = 25 Apr; day46 = 10 Jun 2016).") +
...
```{r}
a <- 
  subset(Fatty_acids, Day == 0 & Forage == "Wheat") %>% 
  mutate(other = C221 + C201)

mean(a$other)

b <- 
  subset(Fatty_acids, Day == 46 & Forage == "Wheat") %>% 
  mutate(other = C221 + C201)

mean(b$other)

c <- 
  subset(Fatty_acids, Day == 0 & Forage == "Rye") %>% 
  mutate(other = C221 + C201)

mean(c$other)

d <- 
  subset(Fatty_acids, Day == 46 & Forage == "Rye") %>% 
  mutate(other = C221 + C201)
...
```{r}
#model
C240 <- lmer(C240 ~ Day*Forage + (1|Pen/Paddock) + (1|Paddock/Forage), 
             data = within(dat, Forage <- relevel(Forage, ref = "Wheat")))

#diagnostics
plot(C240, sqrt(abs(residuals(.))) ~ fitted(.), type = c("p","smooth"))
qqmath(C240)

# check spatial correlation between the 42 pens
simulationOutput <- simulateResiduals(C240)
simulationOutputRecalc <- recalculateResiduals(simulationOutput, group = dat$pen_paddock)
testSpatialAutocorrelation(simulationOutputRecalc, 
                           x =  aggregate(dat$x.cor, list(dat$pen_paddock), mean)$x, 
                           y = aggregate(dat$y.cor, list(dat$pen_paddock), mean)$x,
...
```{r}
{
  ggplot(data = dat, 
    ...
    labs(x = "Grazing interval, day", 
         ...
         caption = "Scatter plot of data with regression (solid lines) and bootstrap 95% confidence intervals (transparent bands) based on \n1000 resamples for lignoceric acid (C24:0) concentrations of fatty acids in rye and wheat forages during the grazing interval \n(day0 = 25 Apr; day46 = 10 Jun 2016).") +
...
```{r}
#model
ratio.2 <- lmer(C182/C183 ~ poly(Day, 2)*Forage + (1|Pen/Paddock) + (1|Paddock/Forage), 
                data = within(dat, Forage <- relevel(Forage, ref = "Wheat")))

#diagnostics
plot(ratio.2, sqrt(abs(residuals(.))) ~ fitted(.), type = c("p","smooth"))
qqmath(ratio.2)

# check spatial correlation between the 42 pens
simulationOutput <- simulateResiduals(ratio.2)
simulationOutputRecalc <- recalculateResiduals(simulationOutput, group = dat$pen_paddock)
testSpatialAutocorrelation(simulationOutputRecalc, 
                           x =  aggregate(dat$x.cor, list(dat$pen_paddock), mean)$x, 
                           y = aggregate(dat$y.cor, list(dat$pen_paddock), mean)$x,
